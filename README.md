...
# 📚SE APROFUDANDO MAIS:
## ( 0 ) OPERADORES:
Os operadores em linguagens de programação são símbolos especiais ou palavras-chave que executam operações em valores ou variáveis. Eles permitem manipular e combinar dados para realizar cálculos, comparações e outras operações. Aqui estão alguns dos operadores mais comuns encontrados em muitas linguagens de programação:
...
### ( 4.1 ) ARRAY: 
É uma variável composta que pode armazenar uma sequência de valores do mesmo tipo em uma única variável. Cada valor no array pode ser acessado por seu índice, que começa em 0.
````
...
### ( 4.4 ) MATRIZES:
São semelhantes às arrays, mas permitem armazenar valores em uma ou mais dimensões. As tuplas são semelhantes às listas, mas são imutáveis, o que significa que não podem ser alteradas depois de criadas.
````
